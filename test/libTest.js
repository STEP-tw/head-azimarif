...
describe('invalidCountMessage', () => {
  it('should return message with line and count', () => {
    ...
    assert.deepEqual(invalidCountMessage({ command: 'head', option: 'n', count: -1 }), expectedOutput);
  });

  it('should return message with line and count', () => {
    let expectedOutput = 'head: illegal byte count -- -1';
    assert.deepEqual(invalidCountMessage({ command: 'head', option: 'c', count: -1 }), expectedOutput);
  });

  it('should return message with line and count', () => {
    let expectedOutput = 'tail: illegal offset -- -1';
    assert.deepEqual(invalidCountMessage({ command: 'tail', option: 'n', count: -1 }), expectedOutput);
  });

  it('should return message with line and count', () => {
    let expectedOutput = 'tail: illegal offset -- -1';
    assert.deepEqual(invalidCountMessage({ command: 'tail', option: 'c', count: -1 }), expectedOutput);
  });
});
...
describe('displayUsage', () => {
  ...
  it('should return tail usage message with type', () => {
    ...
    assert.deepEqual(displayUsage({ command: 'tail', option: '-' }), expectedOutput);
  });
});
